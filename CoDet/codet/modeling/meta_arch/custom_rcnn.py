...
class CustomRCNN(GeneralizedRCNN):
    ...
    def __init__(
            ...
            freq_weight=None,
            roi_heads_in_features = [], # 从backbone中选取的features的来源
            save_features = 'no', # 是否在可视化时仅仅保存features    
            **kwargs):
        """
        """
        ...
        self.roi_heads_in_features = roi_heads_in_features       
    ...
    def from_config(cls, cfg):
        ...
        ret.update({
            ...
            'roi_heads_in_features': cfg.MODEL.ROI_HEADS.IN_FEATURES,
    ...
    def save_features_only(
        ...
        """
        定义模型的推理(inference)方法
        """
        assert not self.training # 执行推理时确保模型不在训练模式
        assert detected_instances is None  # 确保不传入预先检测到的实例

        # Normalize, pad and batch the input images.
        images = self.preprocess_image(batched_inputs)
        # 通过backbone提取特征
        # 特征是一个list，存储着各层backbone的输出特征
        features = self.backbone(images.tensor)
        ...
        # res5：rpn（默认配置）
        # swin: CenterNet      
        if self.save_features == 'coco':
            proposals, _ = self.proposal_generator(images, features, None)
            image_proposal = []
            image_proposal.append(self.roi_heads.get_top_proposals(proposals)[-1][-1])
            in_features = self.roi_heads_in_features # ['res4'] # ovcoco只使用res4
            # print("[features[f] for f in in_features][0].shape: ", [features[f] for f in in_features][0].shape)
            # print("image_proposal[0]", image_proposal)# 
            proposal_boxes = [x.proposal_boxes for x in image_proposal]
            # print("proposal_boxes: ", proposal_boxes)
                    
            image_feature = self.roi_heads._shared_roi_transform([features[f] for f in in_features], proposal_boxes)
            ...
            use_vtol = False
            if use_vtol:
                image_feature = self.roi_heads.box_predictor.cls_score.linear(image_feature)
            # print("image_feature.shape: ", image_feature.shape)
            
        elif self.save_features == 'lvis':
            proposals, _ = self.proposal_generator(images, features, None)
            # proposals是一个list,  且len为1，疑似评估的时候是一张一张评估的
            for proposal_per_img in proposals:
                # proposal_per_img是一个Instances实例，可直接调用其remove api
                proposal_per_img.remove("scores")
                proposal_per_img.remove("pred_classes")
            
            image_proposal = []
            image_proposal.append(self.roi_heads.get_top_proposals(proposals)[-1][-1])
            in_features = self.roi_heads_in_features # ['p3', 'p4', 'p5']        
            
            image_feature = self.roi_heads._get_all_stage_features(features, image_proposal)
            # print("len(image_feature): ", len(image_feature))
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        """
        # 如果是inference模式，则直接使用inference方法
        if self.save_features != 'no':
            return self.save_features_only(batched_inputs)
