...
class CoDetCascadeROIHeads(CascadeROIHeads):
    ...
    def _get_stage_features(self, features, proposals, stage):
        """
        在模型的一个特定阶段上运行前向传播, 
        feature不会向下一层传播, 每一层都是使用的相同的未被上一层加工的feature
        Support classifier_info and add_feature_to_prop
        """

        # 从提议中汇集特征
        pool_boxes = [x.proposal_boxes for x in proposals] # 这里只有一个proposal
        # print("pool_boxes: ",len(pool_boxes))
        box_features = self.box_pooler(features, pool_boxes) # [1,256,7,7]
        # 如果features和pool_boxes的长度不一样，会怎么返回后面的内容呢，会不会只用到了其中的一个？
        # print("box_features.shape: ", box_features.shape)
        # 但这里的features不应该是一级的features吗，这是把所有的都传过来了？
        # box_pooler会根据box的大小，选择一个合适的pooler，也只使用一个特征
        
        # 缩放梯度
        box_features = _ScaleGradient.apply(box_features, 1.0 / self.num_cascade_stages)
        # 通过box_head运行特征, box_head就是FastRCNNConvFCHead
        # 这里就不用自己对长宽维度取平均了,已经使用box_head的全连接层映射为1024维了
        box_features = self.box_head[stage](box_features) # [1,1024]
        
        use_vtol = False
        if use_vtol:
            box_features = self.box_predictor[stage].cls_score.linear(box_features)
        
        return box_features.cpu()
    
    def _get_all_stage_features(self, features, proposals):
        # 根据配置中指定的特征名称获取特征
        features = [features[f] for f in self.box_in_features] # 这里features的长度是3
        ...
        # 遍历所有级联阶段
        for k in range(self.num_cascade_stages):
            predictions.append(self._get_stage_features(features, proposals, k))
        # [3,1,1024]
        return predictions
